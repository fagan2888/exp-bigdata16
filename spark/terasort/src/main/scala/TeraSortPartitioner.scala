...
case class TeraSortPartitioner(numPartitions: Int) extends Partitioner {
  ...
  val rangePerPart = (max - min) / numPartitions + 1

  override def getPartition(key: Any): Int = {
    val b = key.asInstanceOf[Array[Byte]]
    val prefix = Longs.fromBytes(0, b(0), b(1), b(2), b(3), b(4), b(5), b(6))
    // big-endian
    // val prefix = Ints.fromBytes(0, b(1), b(2), b(3))
    // val prefix = Ints.fromBytes(b(2), b(1), b(0), 0)
...
//   val min = Ints.fromBytes(0, 0, 0, 0)
//   val max = Ints.fromBytes(0, -1, -1, -1)  // 0xff = -1
// }
