...
// create files smaller than the block size in the cluster, default is 125MB (block size is 128MB)
...
object BlockTeraGen {
  def main(args: Array[String]) {
    ...
    var numSplits: Int = 63000000
    if(args.length > 2) numSplits = args(2).toInt
    ...
    val conf = new SparkConf()
      .setAppName(s"LineTeraGen ($size)")
    val sc = new SparkContext(conf)

    // val parts = (outputSizeInBytes / 63 / 1000 / 1000).toInt
    val parts = (outputSizeInBytes / numSplits).toInt
