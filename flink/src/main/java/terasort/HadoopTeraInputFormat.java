 ...
 * as Text.
 */
public class HadoopTeraInputFormat extends FileInputFormat<Text,Text> {
  ...
  public static void writePartitionFile(final JobContext job, 
    ...
    final HadoopTeraInputFormat inFormat = new HadoopTeraInputFormat();
  ...
  public List<InputSplit> getSplits(JobContext job) throws IOException {
    ...
    if (job.getConfiguration().getBoolean(HadoopTeraScheduler.USE, true)) {
      HadoopTeraScheduler scheduler = new HadoopTeraScheduler(
        lastResult.toArray(new FileSplit[0]), job.getConfiguration());
      lastResult = scheduler.getNewFileSplits();
      t3 = System.currentTimeMillis(); 
      System.out.println("Spent " + (t3 - t2) + "ms computing HadoopTeraScheduler splits.");
    }
    return lastResult;
  }
}
